...
@Tag(
        name = "CRUD REST APIs for Cards in SofiBank",
        description = "CRUD REST APIs in SofiBank to CREATE, UPDATE, FETCH AND DELETE card details"
)
@RestController
...
public class CardController {
    ...
    @Operation(
            ...
            description = "REST API to create new Card inside SofiBank"
    )
    @ApiResponses({
            @ApiResponse(
                    responseCode = "201",
    ...
    public ResponseEntity<CardDto> fetchCard ( @RequestHeader (CORRELATION_ID) String correlationId,
                                                  ...
                                                  @Pattern(regexp ="(^$|[0-9]{10})", message = "Mobile number must be 10 digits")
                                                  String mobileNumber){
        log.debug("fetchCard method started");
        CardDto cardDto = cardService.fetchCard(mobileNumber);
        log.debug("fetchCard method ended");
    ...
    public ResponseEntity<ResponseDto> updateCard (@Valid @RequestBody CardDto cardDto){
        ...
        boolean isUpdated = cardService.updated(cardDto);
