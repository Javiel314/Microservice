...
import com.microservices.loans.dto.LoanContactInfoDto;
...
public class LoanController {
    ...
    public ResponseEntity<LoanDto> fetchLoanDetails(@RequestHeader(CORRELATION_ID) String correlationId,
                                                     ...
                                                     @Pattern(regexp="(^$|[0-9]{10})",message = "Mobile number must be 10 digits")
                                                     String mobileNumber) {
        log.debug("fetchLoanDetails method started");
    ...
    public ResponseEntity<LoanContactInfoDto> getContactInfo(){
        log.debug("Loan Controller contact info invoked {}", LocalDateTime.now());
        return ResponseEntity
                .status(HttpStatus.OK)
                .body(this.loanContactInfoDto);
    }
}
